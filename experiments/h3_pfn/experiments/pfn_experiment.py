...
# ///
#
# Per-Feature Normalized (PFN) Scoring Experiment — H3
# =====================================================
# Tests whether per-feature anomaly decomposition improves top-1% precision
# and recall over the mean-pool rank-norm baseline on the DAS Group RBA dataset.
#
# Pipeline:
#   data  → FastText (single model, per-account corpus, sg=1)
#        → per-user calibration split (proportional: last max(2, n//3) events)
#        → 6 scoring variants all evaluated on identical test events
#
# Scoring variants:
#   mp-raw          — mean-pool cosine distance (raw, no normalization)
#   mp-rank-norm    — mean-pool cosine distance, rank-normalized per-user vs calib
#   pfn-mean        — per-feature cosine distance, rank-norm per-feature, uniform mean
#   pfn-stab        — per-feature cosine distance, rank-norm per-feature, stability-weighted
#   pfn-max         — per-feature cosine distance, rank-norm per-feature, max over features
#   trivial         — exact set-membership (device key in known devices)
#
# Deliberately excluded (not in scope of this PoC):
#   - IDF-style cross-user feature entropy weighting (optional 4th PFN variant)
#   - Learned weights (no labeled ATOs at sufficient granularity for training)
#   - Token OOV handling beyond FastText subword (min_count=1 covers seen tokens)
#   - Per-class stratified bootstrap (single-stratum percentile bootstrap used)
#   - Hyperparameter tuning of FastText config (ROBUST_KWARGS fixed from H2)
#
# Usage:
#   uv run h3_pfn/experiments/pfn_experiment.py
#   uv run h3_pfn/experiments/pfn_experiment.py --smoke
#   uv run h3_pfn/experiments/pfn_experiment.py --full
...
# remaining events build the centroid.
# This handles variable-length RBA histories (MIN_TRAIN_EVENTS=5 → n//3=1 → floor to 2).
# ---------------------------------------------------------------------------
def split_calib(events: list) -> tuple[list, list]:
    ...
    if n_centroid < 1:
        # Edge case: too few events, use 1 for centroid
        n_centroid = 1
        n_calib = n - 1
...
# ---------------------------------------------------------------------------
# Data loading (identical to rba_rerun.py)
# ---------------------------------------------------------------------------
...
def score_mp_raw(users: dict, model) -> tuple[list, list]:
    """Mean-pool cosine distance, no normalization."""
    scores, labels = [], []
    for u in users.values():
...
def score_mp_rank_norm(users: dict, model) -> tuple[list, list]:
    ...
    scores, labels = [], []
    for u in users.values():
        centroid_evts, calib_evts = split_calib(u["train"])
        centroid = compute_centroid(centroid_evts, embed_mp, model)
        calib_dists = np.array([cosine_dist(embed_mp(e, model), centroid) for e in calib_evts])
        for ev in u["test_pos"]:
...
def score_pfn(users: dict, model, aggregation: str = "mean") -> tuple[list, list]:
    ...
    For each user:
    1. Build per-feature centroids (mean of feature embedding over centroid events).
    2. Build per-feature calibration distributions (cosine dist from per-feature centroid).
    ...
    for u in users.values():
        centroid_evts, calib_evts = split_calib(u["train"])
        # Per-feature centroids
        pf_centroids = compute_per_feature_centroids(centroid_evts, model)
        # Per-feature calibration distributions
        pf_calib = {}
        for f in FEATURE_ORDER:
            pf_calib[f] = np.array([
        ...
        # Stability weights: 1 / (std + eps) — features with low variance in
        # calibration are more predictable and thus more informative as anomaly signals.
        if aggregation == "stab":
            stab_weights = {}
            for f in FEATURE_ORDER:
...
def bootstrap_metrics(scores: list, labels: list,
...
def bootstrap_top1pct(scores: list, labels: list,
                      n_boot: int = N_BOOTSTRAP, seed: int = SEED) -> dict:
    """Bootstrap CIs for top-1% precision, recall, F1."""
    rng = np.random.default_rng(seed)
    sc  = np.array(scores)
    lb  = np.array(labels)
    n   = len(sc)
    ...
    for _ in range(n_boot):
        ...
        if lb_b.sum() == 0:
            continue
        m = threshold_metrics_at_top_pct(sc_b.tolist(), lb_b.tolist())
        precisions.append(m["precision"])
        recalls.append(m["recall"])
        f1s.append(m["f1"])
    if not precisions:
        nan6 = {"mean": float("nan"), "lo": float("nan"), "hi": float("nan")}
        return {"precision": nan6, "recall": nan6, "f1": nan6}
    def ci(arr):
        return {
...
def plot_summary_auc(results: dict) -> None:
    ...
    for ax, metric in zip(axes, ["roc_auc", "pr_auc"]):
        ...
        ax.set_ylabel(label)
        ax.set_title(f"H3 PFN: {label}\n(RBA dataset, sg=1, per-account corpus)")
        ax.axhline(0.5, color="gray", linestyle="--", linewidth=0.8, alpha=0.6)
    plt.tight_layout()
    save_fig(fig, "pfn_summary_auc.png")
...
def plot_top1pct(results: dict) -> None:
    ...
    for ax, metric in zip(axes, ["precision", "recall"]):
        ...
        ax.set_xticks(x)
        ax.set_xticklabels(variants, rotation=20, ha="right", fontsize=9)
        ax.set_ylim(0, min(1.18, max(his) * 1.3 + 0.05) if not np.isnan(max(his)) else 0.20)
        ax.set_ylabel(f"Top-1% {metric.capitalize()}")
        ax.set_title(f"H3 PFN: Top-1% {metric.capitalize()}\n(RBA dataset, 50/50 temporal split)")
        ax.axhline(0.0, color="gray", linestyle="-", linewidth=0.5)
    plt.tight_layout()
    save_fig(fig, "pfn_top1pct.png")
...
def plot_precision_recall_curve(results_raw: dict) -> None:
    ...
    ax.set_title("H3 PFN: Precision-Recall Curves\n(RBA dataset, ATO label)")
    ax.legend(fontsize=8)
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1.05)
    plt.tight_layout()
    save_fig(fig, "pfn_pr_curves.png")
...
def plot_feature_importance(users: dict, model) -> dict:
    ...
    for f in FEATURE_ORDER:
        ...
        for u in users.values():
            centroid_evts, calib_evts = split_calib(u["train"])
            fc = np.mean([embed_feature(e, f, model) for e in centroid_evts], axis=0)
            calib_d = np.array([cosine_dist(embed_feature(e, f, model), fc) for e in calib_evts])
            for ev in u["test_pos"]:
    ...
    ax.set_title("H3 PFN: Per-Feature AUC\n(shows which features carry ATO signal)")
    ax.legend()
    plt.tight_layout()
    save_fig(fig, "pfn_feature_importance.png")
    return feature_aucs
...
# ---------------------------------------------------------------------------
def main() -> None:
    parser = argparse.ArgumentParser(description="H3 PFN experiment on RBA dataset")
    parser.add_argument("--smoke", action="store_true",
                        help=f"fast check on {SMOKE_N_USERS} users (AUC may be NaN)")
    ...
    print("=" * 68)
    print("H3 Per-Feature Normalized (PFN) Scoring Experiment")
    ...
    calib_sizes = [len(split_calib(u["train"])[1]) for u in users.values()]
    centroid_sizes = [len(split_calib(u["train"])[0]) for u in users.values()]
    ...
    for name, (sc, lb) in variants.items():
        roc_m, roc_lo, roc_hi, pr_m, pr_lo, pr_hi = bootstrap_metrics(sc, lb)
        top1 = threshold_metrics_at_top_pct(sc, lb)
        top1_boot = bootstrap_top1pct(sc, lb)
        results[name] = {
            ...
            "top1pct": {
                ...
                "f1":        top1_boot["f1"],
                "point": top1,  # point estimate with TP/FP counts
            },
        }

    # Print summary tables
    print(f"\n  {'Model':<14} {'ROC-AUC':>10} {'[95% CI]':>22}  {'PR-AUC':>8} {'[95% CI]':>22}")
    print("  " + "-" * 84)
    for name in results:
        r = results[name]
        ...
        print(f"  {name:<14} {roc:>10.4f} {ci_r:>22}  {pr:>8.4f} {ci_p:>22}")

    print(f"\n  Top-1% Threshold Metrics (point estimate + bootstrap CI):")
    print(f"  {'Model':<14} {'TP':>4} {'FP':>6} {'Flagged':>8}  "
    ...
    for name in results:
        ...
        print(f"  {name:<14} {pt['tp']:>4} {pt['fp']:>6} {pt['flagged']:>8}  "
    ...
    print(f"\n  HEADLINE:")
    print(f"  mp-rank-norm top-1%: precision={mp_rn_prec:.4f}, recall={mp_rn_rec:.4f}")
    pfn_prec = results[best_pfn]["top1pct"]["precision"]["mean"]
    pfn_rec  = results[best_pfn]["top1pct"]["recall"]["mean"]
    print(f"  best PFN ({best_pfn}): precision={pfn_prec:.4f}, recall={pfn_rec:.4f}")
    verdict = "H3 HYPOTHESIS CONFIRMED" if pfn_beats_baseline else "H3 HYPOTHESIS NOT CONFIRMED"
    print(f"  {verdict}")
    print(f"  Trivial ROC-AUC: {triv_roc:.4f}  |  Best-PFN ROC-AUC: {best_pfn_roc:.4f}")
    ...
    metrics_path = FIGURES_DIR / "pfn_metrics.json"
