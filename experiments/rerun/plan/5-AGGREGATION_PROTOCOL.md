...
Run after all 5 seeds complete. Produces the numbers that go in the paper.

### Primary metric aggregation

For every scalar metric M across seeds [s1..s5]:
...
For bootstrap CIs, report the mean of the lower bounds and mean of the upper bounds
across seeds. Also report whether any seed produces a CI that includes zero — this is the
key stability check for T1.

### Verdict stability

For each pre-specified test verdict, count how many of the 5 seeds produce the same
verdict as the original:

```
verdict_stability = n_seeds_agreeing / 5
```

Any test where verdict_stability < 5/5 requires explicit discussion in the paper.
Any test where verdict_stability ≤ 3/5 (majority flip) requires reframing the claim.

**Target:** 5/5 on all primary verdicts. If any seed flips a verdict, investigate before
submitting — do not average over a flip.
