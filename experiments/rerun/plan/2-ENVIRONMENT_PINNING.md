...
Each experiment script is self-contained using the PEP 723 `# /// script` inline dependency
convention. Run any script with:

```bash
uv run experiments/<script>.py
```

`uv` creates an isolated environment from the script's inline `# /// script` block — no
virtualenv setup, no shared lockfile.

**Before any rerun, verify that every experiment script pins exact versions** in its
`# /// script` block:
...
Minimum packages to pin per script:
- Python `requires-python` (exact minor version)
- gensim (FastText)
- numpy
- scikit-learn (ROC-AUC, PR-AUC, silhouette)
- scipy (bootstrap)
- polars (data handling)

**All experiment scripts must pin identical versions across H2, H6, and RBA.** Do not
update any pinned version between seed runs.
