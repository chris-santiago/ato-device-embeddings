...
- [ ] All 5 seeds complete for H2, H6, RBA
- [ ] Consistency checks passed with no failures
- [ ] Aggregate CSVs generated and spot-checked against per-seed JSONs
- [ ] All verdict stabilities are 5/5 (or exceptions documented)
- [ ] All experiment scripts have exact versions pinned in their `# /// script` blocks
- [ ] Figures regenerated from aggregate (not from individual seed runs)
- [ ] RBA framing uses "open-vocabulary synthetic data with real-world
      distributional structure" throughout — not "real-world replication"
- [ ] PR-AUC reported alongside ROC-AUC in every results table
- [ ] T8 check result included in every experiment's reported diagnostics
- [ ] Seed sensitivity statement drafted for Limitations section
- [ ] arXiv submission prepared before ICAIF August 2 deadline
