...
## 7. What to Document During Each Run

For each seed × experiment, capture to logs/:

- Exact command invoked (with all arguments explicit, no defaults)
- Timestamp start and end
- Library versions (as pinned in the script's `# /// script` dependency block)
- FastText training loss curve (final epoch loss at minimum)
- Vocabulary size actually constructed
- Number of training events per account (mean, min, max)
- T8 check result before any downstream metrics are computed

This is not optional documentation — it is the reproducibility record.
