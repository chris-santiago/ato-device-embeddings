...
**Trivial baseline:** Exact 6/6 feature set-membership check. Achieves 0.791 on spoof by construction: it correctly flags all spoof events (different tz = not an exact training match) and correctly accepts all known-device repetitions, but incorrectly flags a portion of enrollment negatives.
...
All primary results are from H2_RERUN (`robust_config_experiment.py`), using the canonical sg=1 + per-account corpus configuration. Supplemental diagnostics (T4, T6, T8) were run separately on the same dataset under both configurations for comparison. Bootstrap CIs use N=1,000 bootstrap samples, percentile method.

### 2.1 Primary AUC Results

| Condition | Novel AUC | Fleet AUC | Spoof AUC |
|-----------|-----------|-----------|-----------|
| **Mean-pool (robust)** | **0.993** | **0.939** | **0.818** |
| Concat (robust, w=1) | 0.981 | 0.933 | 0.763 |
| Concat (robust, w=6) | — | — | 0.787 |
| Trivial baseline | — | — | 0.791 |

Mean-pool outperforms concat on all three attack types under the robust configuration. The spoof gap is the most operationally significant: mean-pool (0.818) beats the trivial set-membership baseline (0.791), while concat w=1 (0.763) falls below it. Concat w=6 reaches 0.787, still below the trivial baseline. Bootstrap CIs for all four primary deltas (spoof AUC, novel AUC, fleet AUC, silhouette) exclude zero.
...
|--------|-----------|--------------|--------------|
| Spoof AUC | 0.818 | 0.763 | [+0.034, +0.077] |
...
**Results (concat spoof AUC by window):** w=1: 0.763, w=6: 0.787. The best window recovers only 43.6% of the delta to mean-pool (0.818). No window closes the gap.
...
**Test:** Compare mean-pool and concat spoof AUC to the trivial baseline (0.791).

**Results:**
- Mean-pool (robust): **0.818** (+0.027 above trivial)
- Concat w=1 (robust): **0.763** (−0.028 below trivial)
- Concat w=6 (robust): **0.787** (−0.004 below trivial)

**Verdict: Defense wins — for mean-pool only.** This is the operationally critical finding. For the hardest attack scenario (5/6 feature match), mean-pool is the only encoding that adds value over a lookup table. Concat w=1 and w=6 both score below the trivial baseline, meaning they produce worse spoof rankings than a simple membership check. The margin for mean-pool is real (+0.027) but thin — spoof detection is partially addressed, not solved.
...
**Results:** Every tz position in the concat string produces spoof AUC lower than mean-pool (0.818). No reordering recovers more than 50% of the delta. The contamination is cumulative across all feature boundaries — it does not arise from any one boundary position.
...
| T7 (Trivial baseline) | H2_RERUN | Defense wins — mean-pool beats trivial (0.818 > 0.791); concat does not |
...
T7 is the practical test. An embedding approach that cannot beat a two-line hash lookup on the hardest attack type is not operationally justified. Mean-pool clears that bar (0.818 > 0.791); concat at any window does not. The margin is real but thin — spoof detection is partially addressed, not solved.
...
1. **Spoof detection is partially addressed.** Mean-pool spoof AUC 0.818 vs. trivial 0.791 — the margin is real (+0.027) but thin. This is not production-grade spoof detection; it is a meaningful improvement over a lookup table, but the attack surface remains substantial.
...
3. **Spoof margin is thin:** The +0.027 advantage over trivial is statistically real but operationally narrow. Production requirements should be defined before treating this as sufficient.
...
- **Spoof AUC:** Mean-pool 0.818 > Concat 0.763 (w=1). Bootstrap CI for delta: [+0.034, +0.077]. Mean-pool beats trivial (0.791); concat does not.
...
**Fallback:** Exact set-membership check (AUC 0.791 on spoof). If T8 fails, fall back to the trivial baseline rather than serving a collapsed model.

**Do not build:** A concat FastText spoof detector. Concat w=1 achieves spoof AUC 0.763, below the trivial baseline (0.791). No window configuration closes the gap.

### Next Steps

**Immediate:** Spoof detection is partially solved — mean-pool beats trivial (0.818 > 0.791) but the margin is thin. The natural successor is per-feature-dimension anomaly detection (H3): one anomaly score per feature dimension, aggregated by max rather than mean distance. T4 confirms that tz-attributable distance is the signal for spoof attacks; a model that computes this explicitly rather than pooling it into a single device vector should improve spoof AUC substantially.
...
| R2 MINOR-2 | Production implication over-claimed | Fixed: Spoof margin noted as thin (+0.027); limitations section expanded |
...
3. Confirm that the +0.027 spoof margin over trivial is acceptable for the production risk tolerance, or whether H3 investigation should be completed first.
